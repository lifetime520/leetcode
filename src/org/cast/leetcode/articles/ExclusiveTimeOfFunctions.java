package org.cast.leetcode.articles;
...
public class ExclusiveTimeOfFunctions {
	public static String[] pattern1 = {"0:start:0", "1:start:2", "1:end:5", "0:end:6"};
	public static String[] pattern2 = {"0:start:0", "1:start:2", "1:end:5", "2:start:6", "2:end:7", "0:end:9"};
	public static String[] pattern3 = {"0:start:0", "0:start:2", "0:end:5", "0:start:6", "0:end:6", "0:end:7"};

	public static void main(String[] args) {
		List<String> list = new ArrayList<>();
		Arrays.stream(pattern1).forEach(str -> list.add(str));
		System.out.println(Arrays.toString(new Solution().exclusiveTime(2, list))); // [3, 4]

		Arrays.stream(pattern2).forEach(str -> list.add(str));
		System.out.println(Arrays.toString(new Solution().exclusiveTime(3, list))); // [4, 4, 2]

		Arrays.stream(pattern3).forEach(str -> list.add(str));
